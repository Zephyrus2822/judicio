          ...
          {crimebtn.map((crimebtnn, i) => {
            return (
              <div key={i} className="btn mb-5">
                <button onClick={()=>setindex(i)}>{crimebtnn}</button>;
              </div>
            );
          })}
        </div>

        <div className=" border-2 border-black px-5 py-2 crimes">

          {crimes.map((crime,i)=>(
            
            <div key={i} className="crime mb-10 border-[1px] p-2 border-black" hidden={index+1 !== crime.id}>
              <h2 className="text-2xl font-bold">{crime.crime}</h2>
              <p className="text-xl"><span className="font-bold text-xl"> Section:</span> {crime.Section}</p>
              <p className="text-xl"><span className="font-bold text-xl">Bail Status:</span> {crime["Bail Status"]}</p>
              <p className="text-xl"><span className="font-bold text-xl">Bail Amount:</span> {crime["Bail Amount"]}</p>
              <p className="text-xl"><span className="font-bold text-xl">Criteria:</span> {crime.Criteria}</p>
            </div>
          ))}
        </div>
